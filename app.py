...
ORDER_ID_ALIASES = ["sub order no", "sub-order no", "sub_order_id", "suborder no", "sub-order id", "sub order id", "order id", "suborder id"]
PAYOUT_ALIASES = ["final settlement amount", "settlement amount", "bank payout", "bank settlement", "amount transferred to bank", "payout", "net amount", "amount"]
TCS_ALIASES = ["tcs", "tcs amount", "tax collected at source"]
TDS_ALIASES = ["tds", "tds amount", "tax deducted at source"]
ADS_ALIASES = ["advertisement", "ad cost", "ad spend", "marketing cost"]
SKU_ALIASES = ["sku", "sku id", "product sku", "seller sku"]
...
if orders_upload and payments_upload:
    try:
        ...
        if not ord_id or not pay_id or not payout_col:
            ...
            # --- CLIENT DYNAMIC PURCHASE SECTION (IMAGE 1 DEMAND) ---
            ...
            # Creating layout rows for SKU entries like the client's app view
            ...
            # --- CLIENT MAIN SCREEN DASHBOARD GRID (IMAGE 2 DEMAND) ---
            st.markdown("---")
            html_grid = f"""
    ...
    st.info("ℹ️ Kripya apni Meesho Orders aur Payments file upload karein taaki exact dynamic client view active ho sake.")
